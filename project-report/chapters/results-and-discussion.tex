...
If no walls are detected the robots will drive in wide circles, displayed in figure \ref{fig:individual-robot-circle}.
...
Figure \ref{fig:individual-wall-drive} shows how the robots follow the wall by driving in small arches while keeping the walls in sensors range.
...
Robot groups also drive in wide circles, but if the group crashes into a wall it will simply turn around and continue.
...
On the other hand, failing at predator avoidance as much more severe consequences as the predator will kill the robot.

Missing energy = kinda bad
Missing predator = very bad
...
But, as can be seen in figure \ref{3-port-guided-allignment-offset}, if all robots at some time step, were to have rotated their connection ports $50^{\circ}$, the same issue of port alignment would still hold.

The second problem robots with 3 ports in this alignment is the possible architecture the robots can form.
