...
std::string gPropertiesFilename = "config/default-wander_bigrobots.properties";
std::string gCurrentBuildInfo   = "Ljubljana build";  // display through "-v[ersion]" command line option -- check/set value in config.h
...
bool handleKeyEvent(Uint8 *keyboardStates)
{
	...
	{				
		...
		if ( keyboardStates[ SDLK_t ] ) // build/dump image with robot trajectories
		{
		...
		if (keyboardStates[ SDLK_f ]) // display agent focus
		{
...
void updateDisplay() // display is called starting when gWorld->getIterations > 0.
{
		...
		{			
			...
			if ( gBackgroundImage != NULL && gNiceRendering )
				...
				if ( gNiceRendering )
					SDL_FillRect( gScreen, &gScreen->clip_rect, SDL_MapRGBA( gScreen->format, 0xFF, 0xFF, 0xFF, 0 ) ); // clear screen if no background image
				else
...
void initRoborobo()
{
	...
	if( initSDL(SDL_HWSURFACE | SDL_DOUBLEBUF ) == false ) // other option: SDL_HWACCEL
	{
		std::cout << "[CRITICAL] cannot initialize SDL." << std::endl << std::endl;
		exit(-2);
	}
...
void updateTrajectoriesMonitor()
{
    ...
    for ( int i = indexStart ; i != indexStop ; i++ ) // test for agents proximity based on localization 
    {
        ...
        // METHOD 1 : RED lines
        if ( r < 0xFF && r > 0x01 )
            putPixel32( gTrajectoryMonitorImage, x, y,  SDL_MapRGBA( gTrajectoryMonitorImage->format, (r-10), 0x00, 0x00, 0 ) );
        else
            if ( r == 0xFF )
                putPixel32( gTrajectoryMonitorImage, x, y,  SDL_MapRGBA( gTrajectoryMonitorImage->format, 0xFE, 0x00, 0x00, 0 ) );
            else
                putPixel32( gTrajectoryMonitorImage, x, y,  SDL_MapRGBA( gTrajectoryMonitorImage->format, 0x00, 0x00, 0x00, 0 ) ); // saturation
        */
        ...
        if ( r > 0x00 )
            if ( r == 0xFF )
                putPixel32( gTrajectoryMonitorImage, x, y,  SDL_MapRGBA( gTrajectoryMonitorImage->format, 0xF0, 0xF0, 0xF0, 0 ) );
            else
                putPixel32( gTrajectoryMonitorImage, x, y,  SDL_MapRGBA( gTrajectoryMonitorImage->format, (r-1), (r-1), (r-1), 0 ) );
        */
        ...
        if ( g > 0x00 )
        {
            if ( g == 0xFF )
            {
                putPixel32( gTrajectoryMonitorImage, x, y,  SDL_MapRGBA( gTrajectoryMonitorImage->format, 0xFF, 0xA0, 0xA0, 0x00 ) );
            }
            else
            {
                putPixel32( gTrajectoryMonitorImage, x, y,  SDL_MapRGBA( gTrajectoryMonitorImage->format, 0xFF, (g-16), (g-16), 0x00 ) );
            }
        }
    }
