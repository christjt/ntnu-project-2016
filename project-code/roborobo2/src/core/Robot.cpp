...
void Robot::stepBehavior(Uint8* __keyboardStates)
{
	_iterations++;
...
void Robot::behaviorUpdate_remoteController(Uint8* __keyboardStates) // user controlled robot
{
...
void Robot::show() // display on screen
{
	...
	if ( gRobotLEDdisplay == true )
	{
			...
			for ( int xTmp = xcenter - dx; xTmp != xcenter + dx + 1 ; xTmp++ )
				...
				{
					putPixel32_secured( gScreen, xTmp - gCamera.x, yTmp + dy - gCamera.y , SDL_MapRGBA( gScreen->format, r, g, b, 0 ) );
				}		
	}
	...
	if ( _wm->getId() == gRobotIndexFocus && ( gUserCommandMode || gRobotDisplayFocus ) ) // && _iterations%10 < 5) 
	{
			...
			for ( int xTmp = xcenter - dx ; xTmp != xcenter + dx + 1 ; xTmp++ )
			{
				putPixel32( gScreen, xTmp - gCamera.x, ycenter - dy - gCamera.y , SDL_MapRGBA( gScreen->format, r, g, b, 0 ) );
				putPixel32( gScreen, xTmp - gCamera.x, ycenter + dy - gCamera.y , SDL_MapRGBA( gScreen->format, r, g, b, 0 ) );
			}

			for ( int yTmp = ycenter - dy ; yTmp != ycenter + dy + 1 ; yTmp++ )
			{
				putPixel32( gScreen, xcenter - dx - gCamera.x, yTmp - gCamera.y , SDL_MapRGBA( gScreen->format, r, g, b, 0 ) );
				putPixel32( gScreen, xcenter + dx - gCamera.x, yTmp - gCamera.y , SDL_MapRGBA( gScreen->format, r, g, b, 0 ) );
			}
	}
		...
		for ( int i = 0 ; i != _wm->_cameraSensorsNb ; i++ )
		{
			...
			if ( _wm->getCameraSensorValue(i,5) < _wm->getCameraSensorMaximumDistanceValue(i)-1 ) //gSensorRange-1 )
            {
                if ( gDisplaySensors == 2 )
                    ...
                    traceRayRGBA(gScreen, int(x1+0.5)-gCamera.x, int(y1+0.5)-gCamera.y, int(x2+0.5)-gCamera.x, int(y2+0.5)-gCamera.y, 192 , 192 , 255 , 255);
            }
			else
            {
 ...
 */
void Robot::traceRayRGBA(SDL_Surface * image, int x1, int y1, int x2, int y2, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
...
int Robot::castSensorRay(SDL_Surface * image, double x1, double y1, double *x2pt, double *y2pt, int __maxValue )
{
	...
	if ( abs(x1-x2) > abs (y1-y2) )
	{
		...
		{
			if ( getPixel32 ( image, x, (int)(yReal+0.5) ) != SDL_MapRGBA( image->format, 0xFF, 0xFF, 0xFF, 0 ) )
			{
				*x2pt = (double)x;
				*y2pt = yReal;
				isCollision = true;
	...
	else
	{
		...
		{
			if ( getPixel32 ( image, (int)(xReal+0.5), y ) != SDL_MapRGBA( image->format, 0xFF, 0xFF, 0xFF, 0 ) )
			{
				*x2pt = xReal;
				*y2pt = (double)y;
				isCollision = true;
